(ns planetscale.main
  "Kotodama WASM entrypoint for the Planetscale clean-room actor (L4) — Clojure port.
  ...
  facts (`planetscale.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "planetscale")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"planetscale.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "planetscale-compat" :tier tier :entities entities} 200])
...
(defn start! [] :planetscale-compat/ready)
